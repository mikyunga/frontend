...
const ScheduleDragForm = ({ onSubmit, initialTimeslots, meetingTimezone }) => {
  ...
  // ✅ initialTimeslots → 사용자가 선택 불가능한 셀로 등록
  useEffect(() => {
    if (!initialTimeslots) return;
  ...
  // ✅ meetingTimezone → 고정된 셀 (일반적인 회의 시간)
  useEffect(() => {
  ...
  const handleMouseDown = (rowIndex, colIndex) => {
    ...
    if (fixedCells.has(`${cellKey}-fix`)) return;
  ...
  const handleMouseMove = (e) => {
    ...
    if (
      ...
      for (
        ...
        // ❌ disabledCells이거나 fixedCells이면 드래그로 선택 불가
        if (disabledCells.has(`${cellKey}-disabled`)) continue;
        if (fixedCells.has(`${cellKey}-fix`)) continue;
                ...
                return (
                  <td
                    ...
                    style={{
                      ...
                      cursor: isDisabled || isFixed ? "not-allowed" : "pointer",
                    }}
                  ></td>
                );
              })}
